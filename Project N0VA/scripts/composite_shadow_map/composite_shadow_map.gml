...
function composite_shadow_map(argument0) {
	...
	if(lightCount == 0) {
		...
		MSG("composite_shadow_map(lights): ignoring action, there are no lights");
		return false;
	}

	if(!shadow_casters_exist()) {
		// There are no shadow casters
		MSG("composite_shadow_map(lights): ignoring action, there are no shadow casters");
		return;
	}
	...
	if(!has_shadow_map) {
		...
		MSG("composite_shadow_map(lights): failed to create global shadow map");
		return false;
	}
	...
	for(var i = 0, firstLight = true; i < lightCount; ++i) {
		...
		if(shadowMap == undefined) {
			...
			if(firstUseOfGlobalShadowMap) {
				...
				if(!has_shadow_map) {
					// Failed to create a shadow map
					MSG("composite_shadow_map(lights): failed to create light shadow map");
					return false;
				}
